...
#include "nullutils.h"
#include "pnull.h"
#include "null/nullelf.h"  // Diubah dari "NullElf/include/nullelf.h"
...
namespace NullProcess {
    ...
    public:
        ...
        pid_t getPid() const { return pid; } // Tambahkan ini
        ...
        // MemFD mode: inject langsung dari buffer memory (no disk trace)
        bool injectLibraryMemfd(const uint8_t* data, size_t size);

        // File→MemFD: baca file lalu inject via memfd (file bisa dihapus setelah)
        bool injectLibraryFromFile(const std::string& path);
