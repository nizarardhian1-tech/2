...
typedef void *(*InvokerMethod)(void *methodPointer, const MethodInfo *methodInfo, void *object,
                               void **arrayParam); // arrayParam == Il2CppArray
...
struct Il2CppObject
{
    ...
    nlohmann::ordered_json dump(std::vector<uintptr_t> &visited, int maxDepth = 1, bool direct = false);
    std::pair<Il2CppObject *, nlohmann::ordered_json> dump(const std::vector<std::string> &paths = {},
                                                           bool noDump = false);
...
struct Il2CppClass
{
    ...
    std::vector<MethodInfo *> getMethods(const char *filter = nullptr, bool includeParents = false);
