...
std::string dump_method(Il2CppClass *klass)
{
    ...
    while (auto method = il2cpp_class_get_methods(klass, &iter))
    {
        ...
        if (method->methodPointer)
        {
            // Pakai GetLibBase(GetTargetLib()) bukan il2cpp_base langsung
            // agar game seperti MLBB yang pakai libcsharp.so offset-nya benar
            uint64_t base    = (uint64_t)GetLibBase(GetTargetLib());
            uint64_t absAddr = (uint64_t)method->methodPointer;
            uint64_t offset  = (base && absAddr > base) ? (absAddr - base) : absAddr;
            outPut << "0x";
            outPut << std::hex << offset;
        }
        else
        {
            outPut << "0x0";
...
void il2cpp_api_init(void *handle)
{
    ...
    {
        // FIX: pakai xdl_addr bukan dladdr.
        // dladdr tidak tahu soal lib yang embedded di split APK (extractNativeLibs=false),
        // sehingga dli_fbase bisa 0 atau salah.
        // xdl_addr membaca load_bias langsung dari phdr — akurat untuk semua kasus.
        xdl_info_t xinfo = {};
        void *cache = nullptr;
        if (xdl_addr((void *)il2cpp_domain_get_assemblies, &xinfo, &cache) != 0 && xinfo.dli_fbase) {
            il2cpp_base = reinterpret_cast<uint64_t>(xinfo.dli_fbase);
            xdl_addr_clean(&cache);
            LOGI("il2cpp_base (xdl_addr): 0x%" PRIx64, il2cpp_base);
        } else {
            // Fallback ke dladdr jika xdl_addr gagal
            xdl_addr_clean(&cache);
            Dl_info dlInfo;
            if (dladdr((void *)il2cpp_domain_get_assemblies, &dlInfo) && dlInfo.dli_fbase) {
                il2cpp_base = reinterpret_cast<uint64_t>(dlInfo.dli_fbase);
                LOGW("il2cpp_base (dladdr fallback): 0x%" PRIx64, il2cpp_base);
            } else {
                LOGE("il2cpp_base: GAGAL dapat base address! RVA offset akan salah.");
            }
        }
    }
    else
...
namespace Il2cpp
{
    void Init()
    {
        // XDL_TRY_FORCE_LOAD: coba force dlopen jika xdl_find gagal
        // Diperlukan untuk game yang pakai extractNativeLibs=false (embedded di APK)
        auto handle = xdl_open("libil2cpp.so", XDL_TRY_FORCE_LOAD);
        if (!handle) {
            LOGE("[Il2cpp::Init] xdl_open libil2cpp.so gagal!");
        } else {
            LOGI("[Il2cpp::Init] xdl_open OK");
        }
    ...
    std::string getUnityVersion()
    {
        ...
        try {
            static auto unityVersion = get_unityVersion->invoke_static<Il2CppString *>();
            if (unityVersion) { static auto str = unityVersion->to_string(); return str; }
        } catch (...) {}
    ...
    std::string getDataPath()
    {
        ...
        try {
            static auto dataPath = get_persistentDataPath->invoke_static<Il2CppString *>();
            if (dataPath) { static auto str = dataPath->to_string(); return str; }
        } catch (...) {}
    ...
    std::string getPackageName()
    {
        ...
        // Coba get_identifier (modern Unity), fallback ke get_bundleIdentifier (Unity < 5.6)
        static auto get_identifier = Application->getMethod("get_identifier");
        if (!get_identifier) get_identifier = Application->getMethod("get_bundleIdentifier");
        if (!get_identifier) return "unknown_package_name";
        try {
            static auto identifier = get_identifier->invoke_static<Il2CppString *>();
            if (identifier) { static auto str = identifier->to_string(); return str; }
        } catch (...) {}
    ...
    std::string getGameVersion()
    {
        ...
        try {
            static auto version = get_version->invoke_static<Il2CppString *>();
            if (version) { static auto str = version->to_string(); return str; }
        } catch (...) {}
        return "unknown_game_version";
    }
    namespace GC
    {
        // ── Signal guard untuk catch SIGTRAP dari il2cpp_stop_gc_world ──────────
        // Jika dipanggil bukan dari Unity main thread, Unity akan raise SIGTRAP.
        // sigsetjmp/siglongjmp memungkinkan kita recover tanpa crash.
        static thread_local sigjmp_buf  s_jmpBuf;
        static thread_local bool        s_inLiveness = false;
        static struct sigaction         s_oldSigtrap;

        static void sigtrap_handler(int sig, siginfo_t *, void *)
        {
            if (s_inLiveness)
            {
                LOGW("[GC] SIGTRAP caught during liveness scan — not Unity main thread, skipping");
                siglongjmp(s_jmpBuf, 1);
            }
            // Bukan dari kita, forward ke handler asli
            if (s_oldSigtrap.sa_sigaction)
                s_oldSigtrap.sa_sigaction(sig, nullptr, nullptr);
        }

        static void installTrapGuard()
        {
            ...
            sigaction(SIGTRAP, &sa, &s_oldSigtrap);
        }

        // ── Liveness scan dengan SIGTRAP guard ──────────────────────────────────
        static std::vector<Il2CppObject *> livenessBelow2021(Il2CppClass *klass)
        {
        ...
        static std::vector<Il2CppObject *> livenessAbove2021(Il2CppClass *klass)
        {
            std::vector<Il2CppObject *> objects;
            auto callback = [](Il2CppObject **arr, int size, void *userdata) {
                auto *v = reinterpret_cast<std::vector<Il2CppObject *>*>(userdata);
            ...
            auto realloc_cb = [](void *ptr, size_t size, void *) -> void * {
                if (ptr && size == 0) { il2cpp_free(ptr); return nullptr; }
            ...
            if (sigsetjmp(s_jmpBuf, 1) == 0)
            {
                ...
                auto state = il2cpp_unity_liveness_allocate_struct(klass, 0, callback, &objects, realloc_cb);
                il2cpp_unity_liveness_calculation_from_statics(state);
            ...
            {
                // SIGTRAP caught — bukan main thread, objects kosong
                objects.clear();
            }
            s_inLiveness = false;
            return objects;
        ...
        // ── gchandle scan — thread-safe, tapi hanya object dengan GC handle ────
        static std::vector<Il2CppObject *> gchandleScan(Il2CppClass *klass)
        {
            struct Ctx { Il2CppClass *klass; std::vector<Il2CppObject *> *objects; };
            std::vector<Il2CppObject *> objects;
            Ctx ctx{ klass, &objects };
            il2cpp_gchandle_foreach_get_target(
                [](void *data, void *userData) {
                    auto *obj = reinterpret_cast<Il2CppObject *>(data);
                    auto *ctx = reinterpret_cast<Ctx *>(userData);
                    if (!obj || !ctx->klass) return;
                    Il2CppClass *objKlass = nullptr;
                    if (!SafeRead<Il2CppClass *>((const void *)obj, &objKlass) || !objKlass) return;
                    if (objKlass == ctx->klass || il2cpp_class_is_assignable_from(ctx->klass, objKlass))
        ...
        std::vector<Il2CppObject *> FindObjects(Il2CppClass *klass)
        {
            ...
            static bool isBelow2021  = UnityVersion::lt(unityVersion, "2021.2.0");
            LOGD("Searching objects for %s", klass->getFullName().c_str());

            if (isBelow2021)
            {
                // Versi lama: liveness langsung, tidak ada stop_gc_world
                auto result = livenessBelow2021(klass);
                LOGD("Found %zu objects (liveness <2021)", result.size());
                return result;
            }

            // Versi baru: coba liveness dengan SIGTRAP guard dulu
            // (bekerja jika dipanggil dari Unity main thread atau thread yang compatible)
            auto result = livenessAbove2021(klass);
            if (!result.empty())
            {
                LOGD("Found %zu objects (liveness >=2021)", result.size());
                return result;
            ...
            // Fallback: gchandle scan (tidak semua object ketemu, tapi tidak crash)
            LOGD("Liveness returned 0, fallback to gchandle scan");
            result = gchandleScan(klass);
            LOGD("Found %zu objects (gchandle fallback)", result.size());
            return result;
        }
        ...
        void KeepAlive(Il2CppObject *object)
        {
            ...
            if (!SystemGC) return;
            static auto mKeepAlive = SystemGC->getMethod("KeepAlive", 1);
            if (!mKeepAlive) return;
            try { mKeepAlive->invoke_static<void>(object); } catch (...) {}
        }
    } // namespace GC
#ifdef USE_FRIDA
    #include "gumpp.hpp"
